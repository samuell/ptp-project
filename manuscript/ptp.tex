...
\todo{in creating the values?}create the values we used, see~\cite{Sun2017}. As a help to the reader we
...
approach, can handle differing sizes of the datasets well\todo{ref}, and so we see no
...
\begin{tabular}{rlp{0.6\textwidth}}
\toprule
& Name     & Description \\
\midrule
%\multirow{31}{*}{\begin{turn}{90}\textsc{Included}\end{turn}}
...
documentation~\cite{CPSignDocs}. Using this parameter the additional compounds 
are only added to the proper training set, thus part of training the underlying SVM, and 
not part of the calibration of the predictions. This removes potential problems of 
`strange' chemistry affecting the calibration of the predictions and have a negative 
influence on the performance of the predictor.
